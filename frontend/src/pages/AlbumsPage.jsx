...
import { AlbumArtwork, MissingArtistName } from "../components/UI/CatalogArtwork";
...
const AlbumsPage = () => {
  ...
  return <div className="sw-catalog-page">
    ...
    <div className="sw-catalog-toolbar"><div className="![position:relative]"><Search size={16} className="![position:absolute] ![left:13px] ![top:14px] ![color:var(--sw-text-tertiary)]"/><input className="![padding-left:39px]" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search albums" /></div><PremiumSelect value={sort} onChange={(e)=>setSort(e.target.value)}><option value="newest">Newest</option><option value="popular">Most played</option><option value="name">A–Z</option></PremiumSelect><button className="sw-secondary-btn" type="button" onClick={()=>{setQuery("");setSort("newest")}}>Reset</button></div>
    {loading && !albums.length ? <CatalogSkeleton count={10}/> : error && !albums.length ? <EmptyState title="Albums could not load" message={error} onRetry={()=>load({targetPage:1})}/> : albums.length ? <div className="sw-catalog-grid">{albums.map((album)=><article className="sw-catalog-card" key={album._id}><div className="sw-catalog-card-art"><AlbumArtwork src={getAlbumCover(album)} alt={album.title||"Album"} loading="lazy" decoding="async"/><button className="art-open" type="button" onClick={()=>navigate(`/album/${album._id}`,{state:{album}})} aria-label={`Open ${album.title}`}/></div><div className="sw-catalog-card-copy"><strong>{album.title}</strong><span><MissingArtistName name={getArtistName(album)} /></span><div className="sw-catalog-card-meta"><small>{album.songs?.length || 0} songs</small><small>{album.releaseDate ? new Date(album.releaseDate).getFullYear() : ""}</small></div></div></article>)}</div> : <EmptyState title="No albums found" message="Try another album title."/>}
