        ...
        <div className="song-premium-art-column">
          <div className="song-now-playing-stage">
            ...
            <div className="song-premium-actions">
              ...
              <button className={`song-round-action !h-11 !w-11 !basis-11 [&>svg]:!h-5 [&>svg]:!w-5 ${liked?"active":""}`} type="button" onClick={toggleLike} disabled={likeBusy} aria-label="Favorite"><Heart size={18} fill={liked?"currentColor":"none"}/></button>
              <button className="song-round-action !h-11 !w-11 !basis-11 [&>svg]:!h-5 [&>svg]:!w-5" type="button" onClick={addCurrentToQueue} aria-label="Add to queue" title="Add to Up Next"><ListMusic size={18}/></button>
              <button className="song-round-action !h-11 !w-11 !basis-11 [&>svg]:!h-5 [&>svg]:!w-5" type="button" onClick={()=>{if(!token)navigate("/account");else{fetchPlaylists?.();setPlaylistOpen(true);}}} aria-label="Add to playlist"><ListPlus size={18}/></button>
              <button className={`song-round-action !h-11 !w-11 !basis-11 [&>svg]:!h-5 [&>svg]:!w-5 ${offlineSaved?"active":""}`} type="button" onClick={toggleOffline} disabled={offlineBusy} aria-label="Save offline">{offlineSaved?<Check size={18}/>:<Download size={18}/>}</button>
            ...
            <div className="song-detail-transport" aria-label="Song position controls">
              ...
              <div className="song-detail-transport-controls">
                <button type="button" className="song-detail-track-control !h-11 !min-h-11 [&>svg]:!h-[22px] [&>svg]:!w-[22px]" onClick={goToPreviousTrack} disabled={liveListenerSeekLocked} aria-label={liveListenerSeekLocked ? "Live room song changes are controlled by the host" : "Previous song"} title={liveListenerSeekLocked ? "Host controls the live song" : "Previous song"}><SkipBack size={19} fill="currentColor" /></button>
                <button type="button" className="song-detail-skip-control !h-11 !min-h-11 [&>svg]:!h-5 [&>svg]:!w-5" onClick={skipDetailBackward} disabled={!isCurrent || liveListenerSeekLocked} aria-label={liveListenerSeekLocked ? "Live room seeking is controlled by the host" : "Go back 30 seconds"} title="Back 30 seconds"><Rewind size={17}/><span>30</span></button>
                <button type="button" className="song-detail-mini-play !h-14 !min-h-14 !w-14 !min-w-14 [&>svg]:!h-7 [&>svg]:!w-7" onClick={handlePlay} aria-label={isPlaying ? "Pause" : "Play"}>{isPlaying ? <Pause size={23} fill="currentColor" /> : <Play size={23} fill="currentColor" />}</button>
                <button type="button" className="song-detail-skip-control !h-11 !min-h-11 [&>svg]:!h-5 [&>svg]:!w-5" onClick={skipDetailForward} disabled={!isCurrent || liveListenerSeekLocked} aria-label={liveListenerSeekLocked ? "Live room seeking is controlled by the host" : "Go forward 30 seconds"} title="Forward 30 seconds"><FastForward size={17}/><span>30</span></button>
                <button type="button" className="song-detail-track-control !h-11 !min-h-11 [&>svg]:!h-[22px] [&>svg]:!w-[22px]" onClick={goToNextTrack} disabled={liveListenerSeekLocked} aria-label={liveListenerSeekLocked ? "Live room song changes are controlled by the host" : "Next song"} title={liveListenerSeekLocked ? "Host controls the live song" : "Next song"}><SkipForward size={19} fill="currentColor" /></button>
              </div>
        ...
        {momentsLoading && !moments.length ? <CatalogSkeleton count={3} rows/> : moments.length ? <div className="song-moment-list">
          {moments.slice(0,24).map((moment) => <article className="song-moment" key={moment._id}>
            ...
            <div className="song-moment-copy"><strong>{moment.user?.username || moment.user?.name || "Listener"}</strong>{moment.body ? <p>{moment.body}</p> : null}
              ...
              {momentReplyOpen === moment._id ? <div className="song-inline-reply !flex !items-stretch !gap-2 [&>input]:!h-11 [&>input]:!text-[14px] [&>button]:!min-w-[64px] [&>button]:!text-[13px]"><input value={momentReplyBody} onChange={(event)=>setMomentReplyBody(event.target.value.slice(0,300))} placeholder={momentReplyTarget?"Reply to this reply…":"Reply to this moment…"}/><button type="button" onClick={()=>replyMoment(moment._id)}>Reply</button></div> : null}
            </div>
          </article>)}
        </div> : <p className="song-muted-copy">No moments yet. Play the song and leave the first reaction.</p>}
      </section>

      <section className="song-below-grid !grid !grid-cols-1 !gap-8 lg:!grid-cols-[minmax(0,1.45fr)_minmax(280px,.65fr)]">
        <div className="song-comments-section min-w-0 w-full">
          <div className="song-section-title !flex !flex-wrap !items-center !gap-3 [&_h2]:!text-[1.35rem]"><div><span className="song-premium-kicker">Community</span><h2>Comments</h2></div><div className="song-comment-heading-actions !ml-auto !flex !items-center !gap-2"><span>{commentTotal}</span><PremiumSelect value={commentSort} onChange={(event)=>setCommentSort(event.target.value)} aria-label="Sort comments"><option value="recent">Recent</option><option value="top">Top</option></PremiumSelect></div></div>
          <form className="song-comment-form !rounded-2xl !p-3 sm:!p-4" onSubmit={submitComment}>
            <textarea className="!min-h-24 !text-[14px] !leading-6" value={commentBody} onChange={(e)=>setCommentBody(e.target.value.slice(0,600))} placeholder={token?"Add a comment… use @username to mention someone":"Sign in to join the conversation"} disabled={!token||commentBusy}/>
            <div className="song-comment-form-footer !flex !flex-wrap !items-center !gap-2"><label className={isCurrent ? "song-time-toggle" : "song-time-toggle disabled"}><input type="checkbox" checked={commentAtTime} onChange={(e)=>setCommentAtTime(e.target.checked)} disabled={!isCurrent||!token}/><Clock3 size={13}/> Attach {isCurrent ? formatDuration(progress) : "playback time"}</label><span className="song-comment-count">{commentBody.length}/600</span><button className="sw-primary-btn !min-h-10 !px-4 !text-[14px]" type="submit" disabled={!token||!commentBody.trim()||commentBusy}>{commentBusy?"Posting…":"Comment"}</button></div>
          </form>
          {commentsLoading&&!comments.length?<CatalogSkeleton count={4} rows/>:comments.length?<div className="song-comments-list min-w-0">{comments.map((comment)=><article id={`comment-${comment._id}`} className="song-comment !grid !grid-cols-[40px_minmax(0,1fr)] !gap-3 !py-4" key={comment._id}>
            <div className="song-comment-avatar !h-10 !w-10">{comment.user?.image?<img src={comment.user.image} alt=""/>:String(comment.user?.username||"S").slice(0,1).toUpperCase()}</div>
            <div className="song-comment-body min-w-0"><div className="song-comment-top !gap-2 [&>strong]:!text-[14px] [&>span]:!text-[12px] [&>em]:!text-[12px]"><strong>{comment.user?.username||"SoundWave listener"}</strong><span>{new Date(comment.createdAt).toLocaleDateString()}</span>{comment.editedAt?<em>edited</em>:null}</div>
              ...
              {editingId===comment._id?<div className="song-comment-edit"><textarea value={editBody} onChange={e=>setEditBody(e.target.value.slice(0,600))}/><button type="button" onClick={()=>saveEdit(comment._id)}>Save</button><button type="button" onClick={()=>setEditingId("")}>Cancel</button></div>:<p className="!my-2 !text-[14px] !leading-6">{comment.body}</p>}
              <div className="song-comment-actions !flex !flex-wrap !gap-x-4 !gap-y-1 [&>button]:!min-h-9 [&>button]:!text-[12px] [&>button>svg]:!h-4 [&>button>svg]:!w-4"><button type="button" className={comment.liked?"active":""} onClick={()=>likeComment(comment._id)}><Heart size={13} fill={comment.liked?"currentColor":"none"}/> {comment.likes||0}</button><button type="button" onClick={()=>loadReplies(comment._id)}><MessageCircle size={13}/> {comment.repliesCount||0} {comment.repliesCount===1?"reply":"replies"}</button><button type="button" onClick={()=>{if(!token)navigate("/account");else{setReplyOpen(comment._id);setReplyTarget({rootId:comment._id,parentId:comment._id,username:comment.user?.username||"listener"});setReplyBody(`@${comment.user?.username || "listener"} `);}}}><Reply size={13}/> Reply</button>{comment.canEdit?<><button type="button" onClick={()=>startEdit(comment)}>Edit</button><button type="button" onClick={()=>deleteComment(comment._id)}><Trash2 size={13}/> Delete</button></>:null}</div>
              {replyOpen===comment._id?<div className="song-replies-wrap !mt-3 !pl-3">
                {(repliesByComment[comment._id]||[]).map((reply)=><div className={`${reply.parentComment && String(reply.parentComment)!==String(comment._id)?"song-comment-reply nested":"song-comment-reply"} !grid !grid-cols-[32px_minmax(0,1fr)] !gap-2 !py-2`} key={reply._id}><div className="song-comment-avatar small !h-8 !w-8">{reply.user?.image?<img src={reply.user.image} alt=""/>:String(reply.user?.username||"S").slice(0,1).toUpperCase()}</div><div><strong>{reply.user?.username||"Listener"}</strong><p>{reply.body}</p><div className="song-reply-actions"><button type="button" className={reply.liked?"active":""} onClick={()=>likeReply(comment._id,reply._id)}><Heart size={12} fill={reply.liked?"currentColor":"none"}/> {reply.likes||0}</button><button type="button" onClick={()=>{setReplyOpen(comment._id);setReplyTarget({rootId:comment._id,parentId:reply._id,username:reply.user?.username||"listener"});setReplyBody(`@${reply.user?.username||"listener"} `);}}><Reply size={12}/> Reply</button></div></div></div>)}
                <div className="song-inline-reply !flex !items-stretch !gap-2 [&>input]:!h-11 [&>input]:!text-[14px] [&>button]:!min-w-[64px] [&>button]:!text-[13px]"><input value={replyBody} onChange={(event)=>setReplyBody(event.target.value.slice(0,600))} placeholder={token?(replyTarget?.parentId&&String(replyTarget.parentId)!==String(comment._id)?`Reply to @${replyTarget.username || "listener"}…`:"Write a reply…"):"Sign in to reply"} disabled={!token}/><button type="button" disabled={!token||!replyBody.trim()||replyBusy===comment._id} onClick={()=>submitReply(comment._id)}>{replyBusy===comment._id?"Posting…":"Reply"}</button></div>
              </div>:null}
            </div>
        ...
        <aside className="song-recommendations min-w-0 w-full">
