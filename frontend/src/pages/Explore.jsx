  ...
  return (
    <div className="explore-page-v7 sw-container-fluid mx-auto w-full min-w-0 max-w-full overflow-x-hidden px-4 sm:px-6 lg:px-8">
      ...
      {loading && !songs.length ? <section className="explore-results"><CatalogSkeleton count={10} rows/></section> : error && !songs.length ? <section className="explore-results"><EmptyState title="Explore could not load" message={error}/></section> : songs.length ? <>
        ...
        {moodGroups.map((group)=><section className="explore-mood-v7" key={group.name}><div className="explore-section-heading"><div><span className="explore-v7-kicker">Mood</span><h2>{group.name}</h2></div><button type="button" onClick={()=>navigate(`/songs?mood=${encodeURIComponent(group.name)}`)}>See all <ChevronRight size={18}/></button></div><div className="grid grid-cols-2 gap-x-4 gap-y-7 sm:grid-cols-3 lg:grid-cols-6">{group.items.map((song)=><article className="relative min-w-0" key={song._id}><div className="relative aspect-square w-full overflow-hidden rounded-2xl bg-[var(--sw-surface-2)]"><button type="button" className="block h-full w-full" onClick={()=>openSong(song,group.items)} aria-label={`Open ${song.title}`}><SongArtwork className="h-full w-full object-cover" src={getSongCover(song)} alt={song.title || "Song cover"} loading="lazy" decoding="async"/></button><button type="button" className="absolute bottom-2 right-2 grid h-12 w-12 place-items-center rounded-full bg-[var(--sw-accent)] text-white shadow-lg" onClick={()=>playSong?.(song,group.items)} aria-label={`Play ${song.title}`}><Play className="h-6 w-6 translate-x-[1px]" fill="currentColor"/></button><SongActionMenu song={song} queue={group.items} triggerClassName="sw2324-overlay-more" triggerLabel={`More options for ${song.title}`} /></div><strong className="mt-2 block truncate text-[15px] font-extrabold leading-tight text-[var(--sw-text)]">{song.title}</strong><span className="mt-1 block truncate text-[13px] leading-tight text-[var(--sw-text-secondary)]"><MissingArtistName name={getArtistName(song)} /></span></article>)}</div></section>)}
