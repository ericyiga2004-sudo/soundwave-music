...
import { ArrowRight, Headphones, RadioTower, UsersRound } from "lucide-react";
        ...
        <div className="col-12 col-xl-8">
          <section className="sw-social-panel sw20-panel">
            ...
            <div className="sw20-room-grid">
              {(home?.rooms || []).map((room) => (
                <button type="button" className="sw20-room-card" key={room._id} onClick={() => navigate(`/social/rooms/${room.code}`)}>
                  <span className="sw20-room-icon"><Headphones size={20} /></span>
                  <span><strong>{room.name}</strong><small>{room.code}</small><em><UsersRound size={13} /> {room.members?.length || 1} listeners</em></span>
                  <ArrowRight size={17} />
                </button>
              ))}
